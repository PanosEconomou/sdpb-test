...
\end{align*}
Finally using the fact that $\det e^{A} = e^{\text{tr\,}A}$ we can refine this expression to
\begin{equation*}
    \text{Tr\,}\alpha(H) = -Z(2\pi),
\end{equation*}
...
%We can see that $\alpha(E) = \sum_{n=0}^{\lfloor \frac{N}{2} \rfloor} \alpha_n T^n(-2\pi E)$
